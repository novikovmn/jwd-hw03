...
public class Task40 {
	...
	public static void main(String[] args) {
		...
		for (int i = from; i <= number; i++) {
			...
			int numberOfDivisions = 0; // здесь храним количество делений числа на свои цифры (в зависимости от
										// количества нулей)
			if (zero == 0) {
				numberOfDivisions = digits.length;
			} else {
				numberOfDivisions = digits.length - zero;
			}
			...
			for (int j = 0; j < digits.length; j++) { // процесс деления каждого i на свои цифры
				...
				if (digits[j] == 0) { // если цифра числа == 0, то пропускаем ее
					continue;
				}

				if (digits[j] != 0) { // если цифра числа != 0 и i делится на эту цифру, то фиксиреум факт деления
					if (i % digits[j] == 0) {
						counter++;
					} else {
